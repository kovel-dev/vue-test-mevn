		...
		<div class="col-md-10 ">
			...
			<form @submit.prevent="handleUpdateForm">
							...
							<b-list-group-item v-for="provider in defaultProviders" :key="provider._id" class="py-2 d-flex align-items-center justify-content-between">
								<div>
									<input 
										...
										:id="provider._id" 
										:checked="client.providers.findIndex(item => item._id === provider._id) !== -1" 
									/> 
									<label :for="provider._id" class="mb-0">{{ provider.name }}</label>	
								</div> 
				...
				<div class="form-group mt-5 text-right">
					<button class="btn btn-black" @click="closeEditModal">Cancel</button>
					<button class="btn btn-danger">Save Client</button>
				</div>
			</form>
		</div>
	</div>
...
<script>
	...
	export default {
		...
		async created() {
			let apiURL = `http://localhost:4000/api/edit-client/${this.editID}`;

			const apiRes = await axios.get(apiURL)
			this.client = apiRes.data;
			
			let providerURL = `http://localhost:4000/provider/`;

			const providerRes = await axios.get(providerURL)
			this.defaultProviders = providerRes.data;
		},
		methods: {
			handleUpdateForm() {
				let apiURL = `http://localhost:4000/api/update-client/${this.editID}`;
			...
			closeEditModal(e) {
				e.preventDefault()
				this.$root.$emit('bv::hide::modal', 'editModal', '#editClient')
			},
			addProvider(e) {
				e.preventDefault()
